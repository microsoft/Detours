                              ...
                              DWORD nDlls)
{
    ...
    // In this case the file didn't have an import directory in first place, so we couln't fix the missing IAT above.
    // We still need to explicitly give an IAT to prevent to loader from looking for one.
    if (inh.IAT_DIRECTORY.VirtualAddress == 0) {
